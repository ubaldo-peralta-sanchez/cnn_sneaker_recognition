...
X_train, X_test, y_train, y_test = train_test_split(
    X, Y, test_size=0.2)  # asignamos 0.2 al test y 0.8 al train

print('Training data shape : ',
      X_train.shape, y_train.shape)  # print de la forma de la parte del train
print('Testing data shape : ',
      X_test.shape, y_test.shape)  # print de la forma de la parte del test
...
network = Sequential()
# Capa de entrada
network.add(Conv2D(32, kernel_size=(2, 2),
...
network.summary()
# Definir la función de pérdida, el optimizador y las
# métricas para monitorizar el entrenamiento y la prueba de validación
network.compile(optimizer='rmsprop',
                loss='categorical_crossentropy', metrics=['accuracy'])

# Realizar el entrenamiento.
# Guardar el resultado en una variable denominada ‘historia’
history = network.fit(X_train, y_train_cat, epochs=5,
                      batch_size=50, validation_data=(X_test, y_test_cat))
...
for i, correct in enumerate(indices_correctos[:9]):
    plt.subplot(6, 3, i + 1)
    plt.imshow(X_test[correct].reshape(100, 100),
...
for i, incorrect in enumerate(indices_incorrectos[:6]):
    ...
    plt.imshow(X_test[incorrect].reshape(100, 100),
               cmap='gray', interpolation='none')
